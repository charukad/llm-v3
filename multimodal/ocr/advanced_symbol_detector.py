...
# Add MathSymbolDetector class
class MathSymbolDetector:
    """
    Advanced detector for mathematical symbols with contextual understanding.
    
    This class extends the basic SymbolDetector with enhanced capabilities
    for mathematical notation, including detection of special relationships
    such as superscripts, subscripts, and fractions.
    """
    ...
    def detect(image_path: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of detected symbols with positions, relationships, and confidence scores
        """
        return detect_symbols(image_path)
    
    @staticmethod
    def get_expression_structure(symbols: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Analyze the structure of a mathematical expression based on detected symbols.
        
        Args:
            symbols: List of detected symbols
            
        Returns:
            Dictionary describing the expression structure
        """
        # Count different types of relationships
        superscripts = sum(1 for s in symbols if s.get("relation") == "superscript")
        subscripts = sum(1 for s in symbols if s.get("relation") == "subscript")
        numerators = sum(1 for s in symbols if s.get("relation") == "numerator")
        denominators = sum(1 for s in symbols if s.get("relation") == "denominator")
        
        # Determine the main structure type
        structure_type = "simple"  # Default
        if numerators > 0 and denominators > 0:
            structure_type = "fraction"
        elif superscripts > 0 and subscripts > 0:
            structure_type = "notation_with_indices"
        elif superscripts > 0:
            structure_type = "exponentiation"
    ...
    @staticmethod
    def to_latex(symbols: List[Dict[str, Any]]) -> str:
        """
        Convert detected symbols to LaTeX notation.
        ...
        while i < len(symbols):
            ...
            # Check if this symbol has a special relationship
            if "relation" not in symbol:
                latex += text
                i += 1
            elif symbol["relation"] == "superscript" and i > 0:
                latex += "^{" + text + "}"
                i += 1
            elif symbol["relation"] == "subscript" and i > 0:
                latex += "_{" + text + "}"
                i += 1
            else:
                latex += text
