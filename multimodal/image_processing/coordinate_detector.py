...
# Add CoordinateSystemDetector class
class CoordinateSystemDetector:
    """Detector for coordinate systems in images."""
    ...
    def detect(image_path: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary containing detected coordinate system
        """
        return detect_coordinate_system(image_path)
    
    @staticmethod
    def extract_axes(result: Dict[str, Any]) -> Dict[str, Any]:
        """
        Extract axes information from detection results.
        ...
        """
        if not result.get("success", False) or not result.get("has_coordinates", False):
            return {"has_axes": False}
    ...
    @staticmethod
    def has_grid(result: Dict[str, Any]) -> bool:
        """
        Check if the coordinate system has a grid.
        
        Args:
            result: Detection result from detect method
            
        Returns:
            True if the system has a grid, False otherwise
        """
        if not result.get("success", False) or not result.get("has_coordinates", False):
            return False
            
        grid_h = result.get("grid_horizontal", [])
        grid_v = result.get("grid_vertical", [])
        
        return len(grid_h) > 0 and len(grid_v) > 0
